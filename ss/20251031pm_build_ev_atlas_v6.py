...
async function nominatimSuggest(q, which) {{
    ...
    try {{
        ...
        const filteredData = data.filter(d =>
            allowedTypes.includes(d.type) || allowedTypes.includes(d.addresstype)
        );

        const items = filteredData.map(d => {{{{
            const parts = (d.display_name || '').split(',').map(p => p.trim());
            // note the double backslash before d to avoid Python’s \d warning
        ...
        items.sort((a, b) => {{{{
            const aPrec = (a.addr.house_number ? 2 : 0) + (a.addr.road ? 1 : 0);
            const bPrec = (b.addr.house_number ? 2 : 0) + (b.addr.road ? 1 : 0);
            return bPrec - aPrec;
        }}}});
