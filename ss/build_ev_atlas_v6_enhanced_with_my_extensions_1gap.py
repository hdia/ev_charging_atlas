...
#       f"Data source: <b>Open Charge Map API</b>" ,
    ...
    <div id="route-search" style="position: fixed; z-index:100001; top: {ROUTE_PANEL_TOP_PX}px; right: {ROUTE_PANEL_RIGHT_PX}px;
      ...
      font-family:Inter, Arial, sans-serif; font-size:12px; color:#111; width: 400px;">
...
// --- Highlight longest gap section in red (follow actual road) ---
if (maxGap > 300 && chargerCoords.length >= 2) {{
    // Identify which charger pair defines the longest gap
    ...
    for (let i = 1; i < chargerCoords.length; i++) {{
        ...
        if (segKm > longestLen) {{
            longestLen = segKm;
            longestStart = [lat1, lon1];
            longestEnd = [lat2, lon2];
        }}
    }}

    if (longestStart && longestEnd) {{
        // Find where those charger points appear along the route coords
        let startIdx = 0;
        ...
        for (let i = 0; i < coords.length; i++) {{
            const [lon, lat] = coords[i];
            const d1 = haversineKm(lat, lon, longestStart[0], longestStart[1]);
            const d2 = haversineKm(lat, lon, longestEnd[0], longestEnd[1]);
            if (d1 < bestStart) {{ bestStart = d1; startIdx = i; }}
            if (d2 < bestEnd) {{ bestEnd = d2; endIdx = i; }}
        }}

        // Ensure the order is correct along the route
        if (startIdx > endIdx) {{
            const tmp = startIdx;
            startIdx = endIdx;
            endIdx = tmp;
        ...
        // Slice the actual road coordinates for that section
        ...
        if (gapCoords.length > 1) {{
          ...
          if (msg) {{
            ...
            if (maxGap > 300) {{
              msg.innerHTML += `<br><span style='color:red;'>⚠️ Warning: limited chargers for a stretch of approximately ${{maxGap.toFixed(0)}} km.</span>`;
            }}
          }}
        }}
