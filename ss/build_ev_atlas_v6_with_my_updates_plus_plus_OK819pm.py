    ...
    (function() {{
      ...
      whenMapReady(function(mapRef) {{
        ...
        async function doRoute() {{
          ...
          if (msg) msg.innerHTML = `Shortest route between origin and destination found: ${{totalKm.toLocaleString(undefined, {{maximumFractionDigits: 0}})}} km.<br> Chargers within {ROUTE_PROXIMITY_KM:.1f} km along the route are highlighted.`;
        }}
 ...
 document.getElementById('btn-find').addEventListener('click', doRoute);
        document.getElementById('btn-clear').addEventListener('click', function() {{
          ...
          if (chargerCoords.length >= 2) {{
              ...
              // Only one charger found along entire route
              maxGap = totalKm;
          }} else {{
              // No chargers found within PROX_KM of route
              maxGap = totalKm;
          }}
