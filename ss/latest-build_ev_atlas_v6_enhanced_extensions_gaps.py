    ...
    for (let i = 1; i < chargerCoords.length; i++) {{
        ...
        if (segKm > 300) {{
          ...
          if (msg) {{
            ...
            if (maxGap > 300) {{
             msg.innerHTML += `<br><span style='color:red;'>⚠️Route includes section(s) with limited chargers within ${{PROX_KM.toFixed(1)}} km. <br> ⚠️Route includes a road stretch up to ${{maxGap.toFixed(0)}} km  without coverage.</span>`;
            }}
          }}
        }}
