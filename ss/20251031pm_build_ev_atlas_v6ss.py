...
#       f"Data source: <b>Open Charge Map API</b>" ,
    ...
    "Reporting to OCM is voluntary, and some operators may not list all of their chargers or update them regularly.",
...
#    "Use the search box to plot a route and highlight chargers within 5 km of that route.",
    ...
    m.add_child(build_transparent_box("box-howto", howto_html, position="bottomright", offsets=(12,48), width_px=650))
...
(function() {
  ...
  // Create and attach the route planner panel
  const panelHtml = `{panel_html_js_literal}`;
  ...
  async function nominatimSuggest(q, which) {
    ...
    try {
      ...
      const allowedTypes = ['city','suburb','road','house','residential','village','hamlet'];
      const filtered = data.filter(d => allowedTypes.includes(d.type) || allowedTypes.includes(d.addresstype));
      const items = filtered.map(d => {
        const parts = (d.display_name || '').split(',').map(p=>p.trim());
        const cleaned = parts.filter(p => !/Australia/i.test(p) && !/^\\d{4}$/.test(p));
        const label = cleaned.slice(0,3).join(', ');
        return { label, lat:parseFloat(d.lat), lon:parseFloat(d.lon), addr:d.address||{} };
      });
      items.sort((a,b)=>{
        const ap=(a.addr.house_number?2:0)+(a.addr.road?1:0);
        const bp=(b.addr.house_number?2:0)+(b.addr.road?1:0);
        return bp-ap;
      });
      const unique=[]; const seen=new Set();
      for(const it of items){ if(!seen.has(it.label)){ seen.add(it.label); unique.push(it);} }
  ...
  whenMapReady(function(mapRef){
    const routeLayer=L.layerGroup().addTo(mapRef);
    const nearLayer=L.layerGroup().addTo(mapRef);
    let nearHeat=null;
    ...
    async function doRoute(){
      ...
      try{
        ...
        geo=route.geometry;
        totalKm=route.distance?(route.distance/1000):0;
      }catch(e){
      ...
      // Message
      if(msg){
        msg.innerHTML=`Found route (${totalKm.toLocaleString(undefined,{maximumFractionDigits:0})} km).<br>Chargers within {ROUTE_PROXIMITY_KM:.1f} km highlighted.`;
      }
    }
