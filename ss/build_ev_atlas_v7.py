...
def build_map(df, last_refresh, next_refresh):
    ...
    # Summary HTML
    summary_html = f"""
    <div style='font-family: Arial; font-size: 12px; background: white; padding: 8px; border-radius: 6px; box-shadow: 0 0 6px rgba(0,0,0,0.3); width: 300px'>
    ...
    Next refresh: {next_refresh}<br>
    </div>
    """
    folium.Marker([-44.2, 112.9], icon=folium.DivIcon(html=summary_html)).add_to(m)

    # Info box: how to read map
    howto_html = """
    <div style='font-family: Arial; font-size: 12px; background: white; padding: 8px; border-radius: 6px; box-shadow: 0 0 6px rgba(0,0,0,0.3); width: 330px'>
    ...
    """
    folium.Marker([-46, 153], icon=folium.DivIcon(html=howto_html)).add_to(m)
